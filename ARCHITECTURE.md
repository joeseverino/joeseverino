...
`system`, and typed relationships to projects and assets.
...
tokens from one color and monogram — real font-outline rendering, not text in
an SVG. The brand is 100% config; the engine is generic. One source edit
regenerates every brand surface, and the generated output directory is fully
disposable: delete it, rebuild, get byte-identical assets back.
...
cert?" It calls the MCP, which ranks vault docs and returns my actual runbook
— and the answer is the runbook's own commands, not a generated openssl
...
- **Supply chain.** The site repo runs SHA-pinned GitHub Actions: CodeQL,
  ...
  scheduled Lighthouse runs — with the code-scanning dashboard kept at zero
  open alerts.

## The Network Underneath

Nothing private listens on the public internet. Private services ride a
WireGuard mesh (Tailscale) with tailnet lock, private DNS resolution
(AdGuard Home), layered host firewalls, and a private root CA kept on an
offline VM. Monitoring runs *outside* the homelab — an external Uptime Kuma
watches everything over the tailnet, so it catches the failures an internal
monitor would go down with. The only public surface is the static site on
Cloudflare's edge.
...
The build stories behind these systems — screenshots, configs, and the
