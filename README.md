...
- **[severino-vault-mcp](https://github.com/joeseverino/severino-vault-mcp)** - Local-first MCP server that gives AI assistants safe access to an Obsidian operations vault — see a [sanitized sample vault](https://github.com/joeseverino/severino-vault-mcp/tree/main/examples/sample-vault) for how that vault is structured. Layered CI security tooling (CodeQL, pip-audit, OSSF Scorecard, Dependabot), documented threat model, and a four-tier sensitivity gate for credential-adjacent content.
