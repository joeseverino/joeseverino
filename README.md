...
- **[severino-vault-mcp](https://github.com/joeseverino/severino-vault-mcp)** - Local-first server that gives AI assistants safe access to an Obsidian operations vault. The same code path is exposed two ways — as MCP tools for AI sessions and as a cordon-conformant CLI the `site` toolchain wraps — so validation, schema, and atomic writes never drift between the interactive and scripted paths. Layered CI security tooling (CodeQL, pip-audit, OSSF Scorecard, Dependabot), a documented threat model, and a four-tier sensitivity gate for credential-adjacent content.
...
- **[tools](https://github.com/joeseverino/tools)** - Cohesive personal macOS CLI suite: one umbrella command over standalone scripts that share a look, a help convention, and exit-code discipline. Their help, completions, docs, and a full-screen command explorer (`tools tui`) all generate from one cordon declaration per tool, and every measured README claim is asserted by a CI benchmark. Purpose-built TUIs sit on top of the workflow — `brief tui` ranks what needs me now, `repos tui` drives the repo fleet (ship/land/resync), and `site manage` puts every writeup on one screen. It spans age encryption with Keychain-cached unlock, vault sync, dotfile backup, DNS-resolver latency diagnostics, and drift guards that diff live Tailscale, Cloudflare, AdGuard, and Nginx against their vault mirrors.
